...
if __name__ == '__main__':
    ...
    n_epoch = 5
    batch_size = 40 * len(os.environ["CUDA_VISIBLE_DEVICES"].split(','))

    org_train_data, dev_data, test_data = load_dataset('data_add_mrc.pickle')
    print(len(org_train_data))

    for portion in range(9, 11):
        portion = portion / 10
        print(portion)

        temp_train_data = copy.deepcopy(org_train_data)
        train_data = dropout_dataset(temp_train_data, portion)
        train_data = retain_only_positive_examples(train_data, 0.3)
